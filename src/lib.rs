...
use headers::reader::read_bytes_from_file;
use std::ops::Range;
...
pub fn crc32_structure_from_disk<T: Summable + Summable32>(
    ...
    let struct_start = start_offset + summable.range_to_sum().start;
    let struct_size = summable.range_to_sum().end - summable.range_to_sum().start;
