...
pub struct Inode {
    ...
    pub ctime: u32, /* Last inode change time, in seconds since
                     ...
                     * attribute value and this field contains the
                     * lower 32 bits of the attribute values
                     * reference count. */
    pub mtime: u32, /* 	Last data modification time, in seconds
                     * since the epoch. However, if the EA_INODE
                     * inode flag is set, this inode stores an
                     * extended attribute value and this field
                     * contains the number of the inode that owns
                     * the extended attribute. */
    pub dtime: u32, // 	Deletion Time, in seconds since the epoch.
    pub gid: u16,   // 	Lower 16-bits of GID.
    pub links_count: u16, /* Hard link count. Normally, ext4 does not
                     * permit an inode to have more than 65,000
                     * hard links. This applies to files as well
                     * as directories, which means that there
                     * cannot be more than 64,998 subdirectories
                     * in a directory (each subdirectory's '..'
                     * entry counts as a hard link, as does the
                     * '.' entry in the directory itself). With
                     * the DIR_NLINK feature enabled, ext4
                     * supports more than 64,998 subdirectories by
                     * setting this field to 1 to indicate that
                     * the number of hard links is not known. */
    pub blocks_lo: u32, /* Lower 32-bits of "block" count. If the
                         * huge_file feature flag is not set on
                         * the filesystem, the file consumes
                         * blocks_lo 512-byte blocks on disk. If
                         * huge_file is set and EXT4_HUGE_FILE is
                         * NOT set in inode.flags, then the file
                         * consumes blocks_lo + (blocks_hi << 32)
                         * 512-byte blocks on disk. If huge_file
                         * is set and EXT4_HUGE_FILE IS set in
                         * inode.flags, then this file consumes
                         * (blocks_lo + blocks_hi << 32)
                         * filesystem blocks on disk. */
    pub flags: u32,             //
    pub ext_attr_refcount: u32, //sorry hurd
    #[serde(with = "BigArray")]
    pub block: [u8; 60], /* block[EXT4_N_BLOCKS=15] 	Block
    ...
    pub size_hi: u32, /* aka dir_acl 	Upper 32-bits of
                       ...
                       * set to zero and never used. */
    pub obso_faddr: u32, // 	(Obsolete) fragment address.
    pub blocks_hi: u16,  /* 	Upper 16-bits of the block count.
                          * Please see the note attached to
                          * i_blocks_lo. */
    pub file_acl_hi: u16, /* 	Upper 16-bits of the extended
                           * attribute block (historically, the
                           * file ACL location). See the Extended
    ...
    pub ctime_extra: u32, /* 	Extra change time bits. This provides
                           ...
                           * Timestamps section. */
    pub mtime_extra: u32, /* 	Extra modification time bits. This
                           * provides sub-second precision. */
    pub atime_extra: u32, /* Extra access time bits. This provides
                           * sub-second precision. */
    pub crtime: u32, /* 	File creation time, in seconds since the
                      * epoch. */
    pub crtime_extra: u32, /* 	Extra file creation time bits. This
                            * provides sub-second precision. */
...
impl Inode {
    ...
    pub fn filetype_to_str(&self) -> String {
        ...
        match 0xF000 & self.mode {
            ...
            x => {
                panic!(
                    "Error, unknown entry in filemode for inode: \
                     mode was {}",
                    x
                );
            }
    ...
    pub fn get_inode_huge_file_size_bytes(
        ...
        // the file consumes i_blocks_lo 512-byte blocks on
        // disk. If huge_file is set and EXT4_HUGE_FILE_FL is
        // NOT set in inode.i_flags, then the file consumes
        // i_blocks_lo + (i_blocks_hi << 32) 512-byte blocks on disk.
        // If huge_file is set and EXT4_HUGE_FILE_FL IS set in
        // inode.i_flags, then this file consumes (i_blocks_lo
        // + i_blocks_hi << 32) filesystem blocks on disk.
...
pub mod attr_bitflags {
    ...
    pub const EXT4_UNRM: u32 = 0x2; //This file should be preserved, should undeletion be desired .
                                    // (not implemented)
    ...
    pub const EXT4_ENCRYPT: u32 = 0x800; //Encrypted inode . This bit value previously was EXT4_ECOMPR
    ...
    pub const EXT4_DIRSYNC: u32 = 0x10000; //All directory entry data should be written synchronously (see
                                           // dirsync) .
    ...
    pub const EXT4_EA_INODE: u32 = 0x200000; //Inode stores a large extended attribute value in its data
                                             // blocks .
...
pub mod specials {
    ...
    pub const FIRST: u32 = 11; //Traditional first non-reserved
                               // inode. Usually this is the
                               // lost+found directory. See
                               // s_first_ino in the superblock.
}
