...
impl Part {
    ...
    pub fn populate_block_groups(&mut self) {
        let bgdt_offset = self
            .s
            .get_group_descriptor_table_offset(self.start);
        for i in 0..self.s.number_of_groups() {
            if self.s.uses_64bit() && self.s.desc_size > 32
            {
                let combined_size = std::mem::size_of::<
                    BlockGroupDescriptor32,
                >() + std::mem::size_of::<
                    BlockGroupDescriptor64,
                >(
                );
    ...
    pub fn validate_block_groups(&mut self) {
        ...
        for bgid in 0..self.bg.len() {
            ...
            if self.s.metadata_csum() {
                ...
                // crc
                let mut csum_seed: u32 =
                    self.s.checksum_seed;
                ...
                let bg_bytes = <u32>::to_le_bytes(
                    bgid.try_into().unwrap(),
                );
                ...
                let crcsum =
                    summer::crc32c(csum_seed, bytes);
                if bgd_actual.checksum as u32
                    != (crcsum & 0xffff)
                {
                    println!(
                        "{}",
                ...
                // old crc16 version
                let bytesdisk =
                    self.reader.read_bytes_from_file(
