...
pub struct ExtentHeader {
    ...
    pub eh_max: u16, /* 	Maximum number of entries that
                      * could follow the header. */
    pub eh_depth: u16, /* 	Depth of this extent node in
                        * the extent tree. 0 = this
                        * extent node points to data
                        * blocks; otherwise, this extent
                        * node points to other extent
                        * nodes. The extent tree can be
                        * at most 5 levels deep: a
                        * logical block number can be at
                        * most 2^32, and the smallest n
                        * that satisfies 4*(((blocksize
                        * - 12)/12)^n) >= 2^32 is 5. */
    pub eh_generation: u32, /* 	Generation of the tree.
                             * (Used by Lustre, but not
                             * standard ext4). */
}
...
pub struct ExtentLeaf {
    pub ee_block: u32, /* 	First file block number that
                        * this extent covers. */
    pub ee_len: u16, /* 	Number of blocks covered by
                      * extent. If the value of this
                      * field is <= 32768, the extent is
                      * initialized. If the value of the
                      * field is > 32768, the extent is
                      * uninitialized and the actual
                      * extent length is ee_len - 32768.
                      * Therefore, the maximum length of
                      * a initialized extent is 32768
                      * blocks, and the maximum length
                      * of an uninitialized extent is
                      * 32767. */
    pub ee_start_hi: u16, /* 	Upper 16-bits of the block
                           * number to which this extent
                           * points. */
    pub ee_start_lo: u32, /* 	Lower 32-bits of the block
                           * number to which this extent
                           * points. */
}
...
impl ExtentLeaf {
    ...
    pub fn get_file_content(
        ...
        // println!(
        //     "File block: 0x{:X} bytesin_bloc: 0x{:X}
        // size: 0x{:X}",     block, bytes_in_bloc,
        // content_size );
...
impl ExtentTree {
    ...
    pub fn walk(
        ...
        //TODO: this is dumb, loading a huge file right
        // into memory will broke thin
        let mut content: Vec<u8> = vec![];
        //println!("walkng: node depth: {} size_left:
        // {:X}", depth, f_size);
        ...
        //println!("Found content length: {:X}",
        // content.len());
        content
    }
}
